function polygen() {
  ...
  let depth = inch$mm(6)
  let radius = inch$mm(4.25) / 2
  let thickness = inch$mm(0.1)
  let flagstick = inch$mm(0.75) / 2
  ...
  holeCutter.rotation.y = tools.deg$rad(-30)
  ...
  holeCutter.rotation.y = tools.deg$rad(-150)
  let holeTwo = cutMesh(cupExterior, holeCutter)
  holeCutter.rotation.y = tools.deg$rad(-90)
  ...
  baseWall.rotation.y = tools.deg$rad(120)
  baseSupport.rotation.y = tools.deg$rad(120)
  base = joinMesh(joinMesh(base, baseWall), baseSupport)
  baseWall.rotation.y = tools.deg$rad(-120)
  baseSupport.rotation.y = tools.deg$rad(-120)
  base = joinMesh(joinMesh(base, baseWall), baseSupport)
  ...
  cup.rotation.x = tools.deg$rad(-90)
