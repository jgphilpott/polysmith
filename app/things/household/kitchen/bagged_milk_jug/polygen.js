function polygen(language="english") {
  ...
  let wallAngle = tools.deg$rad(trig.angle$sides(null, height, topRadius - bottomRadius))
  ...
  let topCutOffset = (height / 2) + (topRadius / 2) - trig.side$angle(topAngle, null, cylinderOffset + topRadius + thickness, true) - 1
  let topCut = newBox(topRadius * 4, topRadius, topRadius * 4, [0, topCutOffset, 0]).rotateZ(tools.deg$rad(topAngle))
  ...
  let handleTopSupport = cutMesh(newBox(handleThickness, handleHeight, handleRadius * 2, [-(cylinderOffset + topRadius + (handleRadius * 2)), -handleThickness, 0]).rotateZ(tools.deg$rad(-handleAngle)), handleCleanup)
  let handleBottomSupport = cutMesh(newBox(handleThickness, handleHeight, handleRadius * 2, [-(cylinderOffset + topRadius + (handleRadius * 2)), handleThickness, 0]).rotateZ(tools.deg$rad(handleAngle)), handleCleanup)

  let handleTopCleanup = newBox(handleThickness * 2, handleHeight, handleRadius * 3, [-(cylinderOffset + topRadius + (handleRadius * 2) + (handleThickness * 1.5)), -(handleThickness * 2), 0]).rotateZ(tools.deg$rad(-handleAngle))
  let handleBottomCleanup = newBox(handleThickness * 2, handleHeight, handleRadius * 3, [-(cylinderOffset + topRadius + (handleRadius * 2) + (handleThickness * 1.5)), handleThickness * 2, 0]).rotateZ(tools.deg$rad(handleAngle))
  ...
  sleep(1000).then(() => {
    ...
    let text = data.meshes[0].rotateY(tools.deg$rad(handleAngle - 90))
    ...
    data.scene.add(milkJug.rotateX(tools.deg$rad(90)))
