function polygen(degree=10, size=50) {
  ...
  let archExterior = newCylinder(size, size, size, [0, 0, 0], black, (steps + 1) * 4).rotateY(tools.deg$rad(45 / (steps + 1)))
  let archInterior = newCylinder(size + 1, size - thickness, size - thickness, [0, 0, 0], black, (steps + 1) * 4).rotateY(tools.deg$rad(45 / (steps + 1)))
