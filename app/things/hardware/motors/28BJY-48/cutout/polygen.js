function polygen(collar=false) {
  ...
  shaft = cutMesh(cutMesh(shaft, cutOne), cutTwo).rotateX(tools.d$rad(- 90))

  let cutout = newBox(cutoutSize / 2, cutoutSize * 2, cutoutSize, [0, cutoutSize / 2, - (cutoutSize / 2)])

  if (collar) {

    collar = newCylinder(collarLength, collarRadius, collarRadius, [0, collarLength / 2, 0]).rotateX(tools.d$rad(- 90))
